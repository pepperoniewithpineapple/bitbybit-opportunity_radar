...
@ui.refreshable
def render_opportunities(on_apply_change: Optional[Callable] = None):
    #  Search check
...
def render_opportunity_card(opportunity: models.Opportunity, on_apply_change: Callable | None):
    #  Handles the opportunity Apply button functions
    def apply_opportunity():
        if not storage.is_applied_opportunity(opportunity.id):
            storage.add_applied_opportunity(opportunity)
    ...
    def apply_button_hover(is_hovered: bool):
        if storage.is_applied_opportunity(opportunity.id):
            if is_hovered:
    ...
    with ui.card().classes("opp-card p-6 w-full gap-2").props("flat"):
        with ui.row().classes("w-full justify-between items-start"):
            ...
            if search_state.sort_by == "Recommended":
                ...
                score_label = ui.label(f"{opportunity.personalisation_score * 100:.1f}").classes(
                    f"text-xs font-bold tracking-wider px-2 py-1 rounded {colour}"
                )
                score_label.tooltip("Personalisation score")
...
@ui.page("/explore")
def explore(app_state_, on_apply_change: Optional[Callable] = None) -> None:
    global app_state
